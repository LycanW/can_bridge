...
| 字段 | 说明 |
|------|------|
| `publish_hz` | 传感器数据从 CAN→ZMQ 的发布频率上限（主循环频率）。`>0` 时为固定频率；`=0` 表示“有多少发多快”（动态）；`=-1` 表示不发布。 |

### 订阅频率 (subscribe_hz)

| 字段 | 说明 |
|------|------|
| `subscribe_hz` | 控制话题（如 `ctrl_mit` / `ctrl_dji`）从 ZMQ→CAN 的处理频率上限。`>0` 时为固定消费频率；`=0` 表示动态频率（调用多快就尝试多快）；`=-1` 表示不订阅/不处理控制指令，本节点仅作为传感器网关。 |
