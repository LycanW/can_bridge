...
use rs_ctrl_os::{init_logging, start_discovery, PubSubManager, TimeSynchronizer, ConfigManager};
use std::path::Path;
use std::sync::Arc;
use std::thread;
...
fn main() -> Result<()> {
    ...
    // 1. Load Config (ConfigManager 支持热重载，dynamic_load_enable=true 时监听文件变化)
    let cfg_path = "configs/config.toml";
    let manager: ConfigManager<DynamicConfig> = ConfigManager::new(Path::new(cfg_path))?;
    let static_cfg = manager.static_cfg().clone();
    let initial_dynamic = manager.get_dynamic_clone();
    ...
    let can_bus = Arc::new(CanBus::new(
        &initial_dynamic.interfaces,
        &initial_dynamic.devices
    )?);
    ...
    if initial_dynamic.mit_auto_enable {
        thread::sleep(Duration::from_millis(500));
        can_bus.enable_all_mit(&initial_dynamic.devices);
    }

    // 5. Config 热重载：ConfigManager 监听文件变化，本线程定期同步到 can_bus
    ...
    thread::spawn(move || {
        loop {
            ...
            let dynamic = manager_update.get_dynamic_clone();
            can_bus_cfg.update_params(&dynamic.devices);
        }
    });

    // 6. Main Loop (ZMQ pub + ZMQ -> CAN)
    // 控制调度频率：用于 tick_control()（周期重发控制帧）
    let control_hz = initial_dynamic
        .interfaces
        .iter()
    ...
    loop {
        ...
        can_bus.tick_control(&dynamic.interfaces);
